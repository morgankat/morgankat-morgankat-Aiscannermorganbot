...
const AI = window.AIZone = {
  cfg: Object.assign({
    ...
    rawFlipOn:false       // flip on EVERY grid/scalping close, no filter (video-style)
  }, loadCfg()),
...
AI.reactToClose=function(a,closedSide,reason){
  ...
  // RAW FLIP MODE — the "video-style" behavior: reverse on every single
  // close, no filter, no structure check, whether it closed via profit-lock
  // OR a clean TP. This is opt-in and OFF by default — it will also flip
  // straight into losing streaks during a real trend, since it has zero
  // read on whether the move is actually done. Compare it side-by-side
  // against the filtered mode below on demo before trusting either one.
  if(AI.cfg.rawFlipOn){
    const next=closedSide==='BUY'?'SELL':'BUY';
    a.forceNextSide=next; a.forceNextSideIsFakeLeg=true;
    log('AI (raw flip): '+closedSide+' closed on '+a.label+' — firing '+next+' immediately, no filter.');
    return;
  }
  if(reason==='protect'){
    const next=closedSide==='BUY'?'SELL':'BUY';
...
function renderPanel(){
  ...
  panel.innerHTML=`
    ...
    ${sw('Raw flip mode (reverse on every close, no filter)','rawFlipOn')}

...
const AI = window.AIZone = {
  cfg: Object.assign({
    ...
    confirmSeconds:12,   // how long a signal must hold before it's trusted
  ...
  zones:{},        // per-symbol: [{top,bottom,type:'demand'|'supply',touches,score}]
...
// ---------------------------------------------------------------
// ZONE DETECTION — swing-pivot clustering on the active timeframe.
// ---------------------------------------------------------------
function detectZones(a){
  ...
  const tol=Math.max(atrVal*0.35, (a.lastPrice||1)*0.0004);
  const pivots=[];
  for(let i=3;i<bars.length-3;i++){
    const w=bars.slice(i-3,i+4);
    if(bars[i].h===Math.max(...w.map(b=>b.h))) pivots.push({price:bars[i].h,type:'supply',t:bars[i].t});
    if(bars[i].l===Math.min(...w.map(b=>b.l))) pivots.push({price:bars[i].l,type:'demand',t:bars[i].t});
  }
  const clusters=[];
  pivots.forEach(p=>{
    let c=clusters.find(c=>c.type===p.type && Math.abs(c.mid-p.price)<=tol);
    if(c){ c.prices.push(p.price); c.mid=c.prices.reduce((x,y)=>x+y,0)/c.prices.length; c.touches++; c.lastT=Math.max(c.lastT,p.t); }
    else clusters.push({type:p.type, prices:[p.price], mid:p.price, touches:1, lastT:p.t});
  });
  const now=bars[bars.length-1].t;
  const px=a.lastPrice||bars[bars.length-1].c;
  const maxDist=atrVal*10;
  const zones=clusters
    .filter(c=>c.touches>=2 && Math.abs(c.mid-px)<=maxDist)
    .map(c=>({
      type:c.type,
      top:Math.max(...c.prices)+atrVal*0.08,
      bottom:Math.min(...c.prices)-atrVal*0.08,
      touches:c.touches,
      recencyHrs:(now-c.lastT)/3600000,
      score:c.touches*10 - (now-c.lastT)/3600000
    }))
...
if(HAVE_CORE){
  ...
  window.executeTrade=executeTrade=function(a,side){
    ...
    if(isFreshEntry && AI.cfg.masterOn && AI.cfg.confirmEntries && !a.pendingIsFakeLeg){
...
function injectUI(){
  injectStyles();
  const pill=document.createElement('button'); pill.id='aiZonePill'; pill.textContent='🤖 AI';
...
function boot(){
  ...
  log('AI Zone Strategist loaded — tap the AI pill to configure. Master switch is OFF by default; nothing it does affects your trades until you turn it on.');
}

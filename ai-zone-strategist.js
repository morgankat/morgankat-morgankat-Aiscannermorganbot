...
const AI = window.AIZone = {
  cfg: Object.assign({
    ...
    autoLot:false,        // REMOVED per request ("remove this risk % system, it's
                           // making my bot not trade") — kept as a disabled flag so
                           // nothing else in the file needs restructuring, but this
                           // can never turn true again from the UI (toggle removed
                           // below) and AI.autoLotSize() below always returns null
                           // now regardless.
    riskPct:1,
...
AI.autoLotSize=function(a,slDist){
  // REMOVED per request — this system was scaling lot size off balance and
  // risk%, and caused real NOT_ENOUGH_MONEY errors as balance grew. Lot
  // size is now always exactly the fixed Lot Size field, full stop.
  return null;
};
...
function renderPanel(){
  ...
  const riskInput=document.getElementById('aiRiskPct'); // no longer rendered — element won't exist; guarded no-op
...
function injectUI(){
  injectStyles();
  const pill=document.createElement('button'); pill.id='aiZonePill'; pill.textContent='🤖 AI v8';
...
function boot(){
  ...
  log('AI Zone Strategist v8 loaded — tap the AI pill (now labeled "AI v8" — check it says v8) to configure. Master switch is OFF by default; nothing it does affects your trades until you turn it on.');
}

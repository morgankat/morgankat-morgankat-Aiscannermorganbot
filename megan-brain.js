  ...
  "riskPercent":number|null
    ...
    async function explainChart() {
        ...
        const instruction = 'Give your own independent professional trader\'s read of this market — do not just restate indicator labels, work it out from the raw candles and/or the chart image yourself, and use real current research for context, not stale knowledge. ' +
            ...
            (has('getAnalysisBrief') ? ((call('getAnalysisBrief') || '') + ' ') : '') +
    ...
    function tryCommand(t) {
        // Optional adapter hook: a bot can claim an utterance before the built-in commands (used by the scanner).
        if (has('voiceCommand') && call('voiceCommand', t) === true) return true;
    ...
    function startListening() {
        ...
        recognition.onresult = (e) => {
            ...
            // FIX ("mic is too sensitive, picks up surrounding noise, jumps
            // on and off"): true single-speaker isolation isn't something
            // the browser's speech API can do at all — there's no setting
            // for it, on this or any web app using the same standard API.
            // What IS available and wasn't being used: every result comes
            // with a confidence score, and this used to process every
            // transcript regardless of how low that score was — so a
            // half-caught fragment of someone else talking nearby, or a
            // burst of background noise the engine took a guess at, went
            // straight through as a command. Low-confidence results are
            // now dropped before they ever reach command handling.
            if (last[0] && typeof last[0].confidence === 'number' && last[0].confidence > 0 && last[0].confidence < 0.55) return;
